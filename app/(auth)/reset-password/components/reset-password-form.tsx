  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!isValidPassword || !passwordsMatch) {
      toast({
        ...
        description: "Please make sure your password meets all requirements and both passwords match.",
        variant: "destructive",
      })
      return
    }
          ...
          <div className="space-y-2">
            ...
            <ul className="space-y-1 text-sm">
              ...
              <li className="flex items-center gap-2">
                ...
                <span className={hasUppercase ? "text-green-700" : "text-red-700"}>At least one uppercase letter</span>
              </li>
              <li className="flex items-center gap-2">
                {hasLowercase ? <Check className="h-4 w-4 text-green-500" /> : <X className="h-4 w-4 text-red-500" />}
                <span className={hasLowercase ? "text-green-700" : "text-red-700"}>At least one lowercase letter</span>
              </li>
              <li className="flex items-center gap-2">
                {hasNumber ? <Check className="h-4 w-4 text-green-500" /> : <X className="h-4 w-4 text-red-500" />}
                <span className={hasNumber ? "text-green-700" : "text-red-700"}>At least one number</span>
              </li>
              <li className="flex items-center gap-2">
