...
export async function POST(req: Request): Promise<NextResponse> {
    try {
        ...
        if (type === "order") {
            ...
            if (!orderItem) {
                return NextResponse.json({ error: "No product found for this order." }, { status: 404 });
            }
            ...
            if (totalReturned > orderItem.quantity) {
                return NextResponse.json({ error: "Return quantity exceeds the ordered quantity." }, { status: 400 });
            }
        } else {
            ...
            if (quantity > product.quantity_in_stock) {
                return NextResponse.json({ error: "Return quantity exceeds available stock." }, { status: 400 });
            }
        }
...
export async function DELETE(req: Request): Promise<NextResponse> {
    try {
        ...
        return NextResponse.json({ message: "Product return deleted, and stock reverted successfully." }, { status: 200 });
    } catch (error) {
        console.error("Error:", error);
        return NextResponse.json({ error: (error as Error).message }, { status: 500 });
    }
